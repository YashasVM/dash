...
function drawPixels(time){if(reduceMotion&&lastDraw){return}if(!reduceMotion&&time-lastDraw<45){return}lastDraw=time;const image=context.createImageData(pixelWidth,pixelHeight),data=image.data,t=reduceMotion?0:time*.0018;
...
function syncPixelMotion(){if(reduceMotion||!smallScreen.matches){if(animationTimer){window.clearInterval(animationTimer);animationTimer=0}return}if(!animationTimer){animationTimer=window.setInterval(function(){drawPixels(performance.now())},50)}}
resizePixels();window.addEventListener('resize',function(){resizePixels();window.clearTimeout(resizeTimer);resizeTimer=window.setTimeout(function(){lastDraw=0;drawPixels(performance.now())},100);syncPixelMotion()},{passive:true});smallScreen.addEventListener('change',function(){resizePixels();lastDraw=0;drawPixels(performance.now());syncPixelMotion()});window.setTimeout(function(){drawPixels(performance.now())},0);syncPixelMotion();
