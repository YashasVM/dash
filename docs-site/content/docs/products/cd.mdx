...
## Agent and terminal sending

Terminals and agents use the [`cdx` CLI](./cd-cli) instead of the browser picker: `cdx send ./file.zip` prints one private link, and the transfer runs over the [agent transfer protocol](./cd-protocol) with end-to-end encryption and receiver-verified completion.
